...
- **Integración de la versión de Rafael (OE3) en el informe (25-sep)**: Rafael editó el informe directamente en Word (`Informe_Avance_ABET_Barcelo_Dede_OE3.docx`, fuera de git) partiendo del commit `33e7d99`. Se compararon los textos de ambos PDF y se portaron sus cambios al script generador: Resumen redactado de nuevo; motor de decisión con cuatro métodos (fijo, rodante, banda, híbrido) y selección por criterio de estabilidad maximin entre mitades del periodo; párrafo del Dashboard; en 7.3, Tablas 18 (nueva versión) y 19, Figuras 5-7 (imágenes extraídas de su docx a `informe_avance_assets/`), sensibilidad de umbrales y traducción a pesos del último mes; cambios coherentes en Tablas 10, 12 (columna "Criterio de implementación") y 13, en 8.4, en las conclusiones de OE3/OE4 y una recomendación 6; y cuatro referencias nuevas (Tukey, Gama et al., Wald, Garivier y Moulines). Tres decisiones: (1) en el Resumen se corrigió la gramática y "seis modelos de predicción por machine learning" quedó como "estadísticos y de aprendizaje automático", porque persistencia, ARX+GARCH y GARCH-ged no son de aprendizaje automático; (2) **no** se aplicó su cambio en la fila OE1 de la Tabla 2 (Alcances), que reemplazaba el alcance por las tres *limitaciones* de OE1 del Anexo 1 §4.2, que ya están en la Tabla 3; (3) su párrafo "Evidencia por objetivo" de 7.4 no se restauró, porque se había quitado a propósito por redundante. También se corrigió un defecto de la plantilla que el Resumen más largo dejaba ver: los saltos de sección forzados (un párrafo vacío de Título 1 más un párrafo con solo un salto de página) se desbordaban y dejaban páginas en blanco (la 3, que siempre había estado, y una nueva tras el Resumen). Ahora se reemplazan por "salto de página antes" en el título siguiente, sin tocar las marcas de campo del índice. Resultado: 25 páginas sin páginas en blanco (antes 23 con una en blanco).
- **6.4 (Alternativas de solución) ampliada y bug de tabla corregido (25-sep, a pedido del usuario)**: las tres descripciones se detallaron (qué hace cada arquitectura conceptualmente: ARX log-lineal + GARCH(1,1) con banda nativa; N-BEATSx con descomposición en bloques y cuantiles calibrados aparte; ensamble con la lógica de diversificación de portafolio), y Ventajas/Desventajas se explicaron un poco más. Además se quitó, a pedido del usuario, la frase "su robustez histórica solo se mide en 2026" de la Desventaja de la alternativa 3 (el hallazgo sigue completo en 7.4 "Lectura honesta de la robustez"). De paso se encontró y corrigió un bug real: la fila "Desventajas" de esta tabla (rellenada fuera del helper `tabla()`, directamente sobre las celdas de la plantilla) se partía a media frase entre dos páginas porque nunca se le puso `w:cantSplit`; corregido ahí y, preventivamente, en las otras dos tablas de la plantilla rellenadas del mismo modo (normas y criterios de decisión).
- **Correcciones puntuales (25-sep, a pedido del usuario) en 5.1 y la portada**: el rango de precio diario 2023-2024 citado en la Introducción decía "entre 124 y 2.499 COP/kWh" — verificado contra los datos crudos, el mínimo real combinado de 2023-2024 es **106,9** (28-abr-2024, no 123,8 de 2023 solo), corregido a "107" con cita a la fuente (XM, ref. [4]). Confirmado además que no contradice el heatmap año-mes del notebook 03 (son promedios mensuales, no extremos diarios); abril-2024 promedia 749 pero cae a 106,9 el día 28, y septiembre-2024 promedia 874 pero salta a 2.498,8 el día 30 (transición al pico de octubre). Tabla 2 (Alcances OE1): quitado "(66.576 h)" de la celda de estado, queda solo "Cumplido".
- **Sección 5.1 y 5.2 ampliadas (25-sep, a pedido del usuario) con la experiencia de diseño y descripciones accesibles**: nuevo párrafo en 5.1 conectando "no tiene solución única" (criterio ABET) con la experiencia real de diseño — 40 variables adoptadas de más de una docena de candidatas descartadas, 6 modelos de 15 familias evaluadas, decisiones sin receta única (Tablas 11-13). En 5.2, dos párrafos nuevos explican en términos simples qué hace cada técnica de los 6 trabajos comparados (NARX, ARIMA+GARCH integrado, FPCA, LEAR/LASSO, MASE) para un lector sin conocimiento de mercados eléctricos, y por qué su resultado no es directamente comparable. Esto llevó el documento de 22 a 23 páginas; se recortó lo redundante (un párrafo "Evidencia por objetivo" en 7.4 que repetía la Sección 9, condensado a solo la parte de interpretabilidad de SHAP que no estaba en otro lado; el cierre de 5.2 que repetía lo ya explicado) pero **el usuario decidió dejarlo en 23 páginas** en vez de seguir sacrificando contenido.
- **Reestructuración (25-sep, a pedido del usuario): metodología de diseño completa y máximo 22 páginas** (quedó en 22, luego 23 por lo anterior). Agregado en 6.8: procedimiento de diseño (alternativa → evaluación fuera de muestra → adopción solo con DM p<0,05 y 10/10 particiones → bitácora); Tabla 11 con las **40 variables** por familia, fórmula y justificación desde OE1 (el "33 variables" del README antiguo no incluía los 7 festivos); Tabla 12 con fundamento, configuración e hiperparámetros de cada modelo y cómo se ajustó; modelos descartados con su rMAE en 2026 (CatBoost 1,016, hurdle 1,028, Markov-Switching 0,990, RLS 1,052, TFT 1,020, LSTM 1,173, LEAR24 0,940; Prophet/LightGBM de una versión anterior); Tabla 13 con 12 decisiones de diseño, alternativa descartada y evidencia — incluida la codificación del ONI (continuo frente a categorías difusas NOAA, que empeoraron N-BEATSx 56,11→58,47; la bandera binaria no se probó, así se dice). Recortado: 3 figuras, tablas de alcances/limitaciones compactadas, entregables en un párrafo, tabla de factores, riesgos a 6, backtest solo con el método elegido, bandas y costo en una tabla, y la tabla de comparación con la literatura (redundante con la Tabla 4).
