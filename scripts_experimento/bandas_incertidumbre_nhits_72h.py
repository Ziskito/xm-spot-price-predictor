# -*- coding: utf-8 -*-
"""
Extension del trabajo de hoy al horizonte completo 24-72h (pedido explicito del usuario antes de
dormir): N-HiTS nativo a h=72 con MQLoss, espejo exacto de bandas_incertidumbre_nbeatsx_72h.py
(que ya existia para N-BEATSx). Mismos regresores, mismo pipeline, mismo protocolo -- para poder
combinar ambos en un ensamble de stacking por tramo de horizonte (1-24h / 25-48h / 49-72h).
"""
import pandas as pd
import numpy as np
import time
...
RUTA_CONTRATO = RAIZ / "data" / "processed" / "resultados" / "pronostico_nhits_72h_2026.csv"
RUTA_METRICAS = RAIZ / "data" / "processed" / "resultados" / "bandas_metricas_nhits_72h.csv"
...
def main():
    ...
    print(f"Entrenando N-HiTS h={HORIZONTE} con MQLoss (cuantiles {CUANTILES})...")
    ...
    cols_pred = [c for c in cv.columns if c.startswith("NHITS")]
    ...
    print(f"\n{'='*70}\nMAE global N-HiTS (cuantil 50, todos los pasos 1-72h): {mae_q50:.2f}")

    filas_met = [{"tramo": "global", "mae": mae_q50, "n": len(salida)}]

    print(f"\n{'='*70}\nDESGLOSE POR TRAMO DE HORIZONTE\n{'='*70}")
    ...
    print(f"\nContrato N-HiTS 24-72h guardado en: {RUTA_CONTRATO}")
